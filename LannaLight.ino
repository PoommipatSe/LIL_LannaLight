...
void timer0_ISR (void) {
  ...
  timer0_write(ESP.getCycleCount() + 10000000L); // 80MHz == 1sec
...
int amplitudeShowCounter = 0; // This is variable to counting rounds of amplitudeShow() which has to be 2 to complete task.

int ampLoopCounter = 0; // this variable is to iterate a loop for counting the idle status of mainMode 1 (amplitudeShow()).
...
void amplitudeShow2(){
  ...
  if(sensorMain <= 100) {
    ...
    ampLoopCounter++;
    if ( ampLoopCounter > 10000){
      //back to normal
      ampLoopCounter = 0;
      amplitudeShowCounter = 0;
      mainMode = 0;
      }
    ...
    if(sensorMain <200 ){
      ...
      rapidBlink();
      delay(10);
      amplitudeShowCounter++;
      
      
      if(amplitudeShowCounter >= 2){
        rapidBlink();
        showEnding();
      }
